...
export const AdminOrders = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'orderStatus',
      render: (item: any) => (
        <Badge dot variant={statusVariant(item.orderStatus)} className="capitalize">
          {item.orderStatus}
        </Badge>
      ),
    },
    {
      ...
      render: (item: any) => (
        <Badge
          variant={item.paymentStatus === 'paid' ? 'success' : item.paymentStatus === 'partial' ? 'warning' : 'default'}
      ...
      render: (item: any) => {
        ...
        return (
          <div className="flex flex-wrap gap-2">
            <Button size="sm" variant="ghost" onClick={() => navigate(`/admin/orders/${item.id}`)}>View</Button>
